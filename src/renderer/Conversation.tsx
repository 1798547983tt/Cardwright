...
function Markdown({ children }: { children: string }) {
  ...
  return useMemo(() => <ReactMarkdown remarkPlugins={[remarkGfm, fileReferences]} urlTransform={url => url.startsWith(FILE_LINK) ? url : defaultUrlTransform(url)} components={{
    a: ({ children: content, href }) => href && href.startsWith(FILE_LINK)
  ...
  }}>{children}</ReactMarkdown>, [children, api, run, view]);
}
...
function Turn({ turn, task, last, editSignal, chapter }: { turn: ConversationTurn; task: Task; last: boolean; editSignal?: number; chapter?: string }) {
  ...
  const assistant = turn.entries.filter((entry): entry is TimelineItem & { type: 'message' } => entry.type === 'message' && entry.item.role === 'assistant');
  ...
  const process = turn.entries.filter(entry => entry !== visibleFinal || (entry.type === 'message' && !!entry.item.thinking));
  const tools = turn.entries.filter(entry => entry.type === 'tool');
  const thinking = assistant.filter(entry => entry.item.thinking).length;
  const actualProcess = process.filter(entry => entry.type === 'tool' || entry.item.thinking || (entry !== visibleFinal && entry.item.text && !(entry.item.role === 'system' && entry.item.usage)));
  ...
  return <section className="conversation-turn" data-turn-id={turn.id}>
    ...
    {hasResponse && <article className={`message message-assistant assistant-turn turn-row is-assistant ${task.truncation && !active && (task.truncation.turnId ? task.truncation.turnId === turn.id : last) ? 'is-truncated' : ''}`}><Avatar role="assistant" size={34} interactive={!task.agentName} agentName={task.agentName} /><div className="turn-card"><div className="message-author"><span>{task.agentName || 'Cardwright'}</span>{active && last && <span className="turn-running-label">{t('Working', '处理中')}</span>}<time dateTime={turn.entries[0].at}>{new Date(turn.entries[0].at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</time></div>
      ...
      {(visibleFinal || usageRecords.length > 0) && <div className="message-footer">{visibleFinal && <IconButton label={t('Copy response', '复制回复')} onClick={() => void run(() => navigator.clipboard.writeText(visibleFinal.item.text), t('Response copied', '已复制回复'))}><Copy size={14} /></IconButton>}{turn.user && <IconButton label={t('Regenerate response', '重新生成回复')} disabled={active || busy} onClick={() => void regenerate()}><RefreshCw size={14} className={busy ? 'spinning' : ''} /></IconButton>}{usageRecords.length > 0 && <UsageDetail usage={usage} />}{visibleFinal?.item.model && <span className="response-model">{visibleFinal.item.model}</span>}</div>}
    </div></article>}
  </section>;
}
